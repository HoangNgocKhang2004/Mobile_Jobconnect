...
import 'package:job_connect/core/models/auth_response.dart';
...
class _LoginScreenState extends State<LoginScreen>
  ...
  goToHomeHR(BuildContext context, Account user) {
    Navigator.pushReplacement(
      ...
      MaterialPageRoute(
        builder:
            (BuildContext context) =>
                RecruiterScreen(isLoggedIn: true, userAccount: user, currentIndex: 0),
      ),
    );
  }
  ...
  // Đăng nhập bằng API
   Future<void> _loginWithApi() async {
    ...
    try {
      final AuthResponse auth = await _apiService.login(dto);
  ...
  Future<void> _loginIn(BuildContext context) async {
    ...
    final userDoc =
        response.isNotEmpty ? Account.fromJson(response.first) : null;
    print(
    ...
    if (userDoc != null) {
      if (mounted) {
        ...
        _animationController.reverse().then((_) {
          ...
          if (userDoc.role != null && userDoc.role.roleName == 'Recruiter') {
