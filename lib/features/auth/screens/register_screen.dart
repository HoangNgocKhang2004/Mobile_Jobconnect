...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  late List<Role> lstRole = [];

  Future<void> fetchRoles() async {
    try {
      final data = await _apiService.get(ApiConstants.roleEndpoint);
      lstRole = data.map((item) => Role.fromJson(item)).toList();
      print(lstRole);
    } catch (e) {
      print('Lỗi khi lấy danh sách vai trò: $e');
  ...
  void initState() {
    super.initState();
    fetchRoles(); // Gọi hàm lấy danh sách vai trò khi khởi tạo
    print(_apiService.baseUrl);
  }
  ...
  Future<void> _email_password_sign_up(BuildContext context) async {
    ...
    if (_formKey.currentState!.validate()) {
      ...
      final userAccount = Account(
        ...
        idRole: _roleController.text == 'Recruiter' ? 'role1' : 'role2',
  ...
  Future<void> _completeGoogleRegistration(BuildContext context) async {
    ...
    if (_roleController.selection == const TextSelection.collapsed(offset: 0)) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Vui lòng chọn vai trò của bạn (Recruiter hoặc Candidate)',
          ),
          backgroundColor: Colors.red,
        ),
      );
